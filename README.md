...
The background is a photograph generated by `scripts/gen-cover.mjs` (Meshy,
`gpt-image-2`, 12 credits, needs `MESHY_API_KEY`). The Meshy MCP server is also
registered at user scope, so the same generation can be driven conversationally
instead of by script — `meshy_text_to_image`, `meshy_image_to_image`,
`meshy_check_balance` and twenty-odd 3D tools. It asks for no lettering
anywhere: generated type is always gibberish, and the title is set in the app
over the book's own cloth instead.
...
app; only `text-to-image`, `image-to-image` and `image-to-3d` answer. So the
cover is a still and the movement is done in the app: the camera drifts about a
percent of the frame over 34 seconds, and the lamp breathes on an 11-second
cycle so the two never pulse together. That also keeps it at ~320KB rather than
the several megabytes a loop would cost, and it cannot stutter. Reduce Motion
holds it still.
