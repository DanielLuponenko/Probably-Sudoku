...
Putting a Book down needs no button: closing the app keeps it, and the shelf
offers to continue. **Abandon Book** is the only exit and it means what it
says — the run is thrown away and the save deleted, behind a confirmation.

It originally did neither: it returned to the shelf and left `run.json` on
disk, so the game offered to continue a Book it had just told the player was
gone.
