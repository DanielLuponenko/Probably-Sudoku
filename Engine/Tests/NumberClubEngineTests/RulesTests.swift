...
final class ObstacleTests: XCTestCase {
    ...
    func testObstacleFourThroughNineAreRealRules() throws {
        ...
        let eightPuzzle = try XCTUnwrap(eight.puzzle)
        XCTAssertEqual(eightPuzzle.obstacleBlockedDigits.count, min(3, Set(eightPuzzle.hand).count - 1))
        XCTAssertTrue(eightPuzzle.hand.indices.contains { !eightPuzzle.isBlocked(handIndex: $0) })

        var nine = Game(seed: "obstacle-nine", obstacle: .finalEdition)
        try nine.startPuzzle()
        XCTAssertEqual(nine.puzzle?.handSize, 4)
        let ninePuzzle = try XCTUnwrap(nine.puzzle)
        XCTAssertEqual(ninePuzzle.obstacleBlockedDigits.count, min(3, Set(ninePuzzle.hand).count - 1))
        XCTAssertTrue(ninePuzzle.hand.indices.contains { !ninePuzzle.isBlocked(handIndex: $0) })
